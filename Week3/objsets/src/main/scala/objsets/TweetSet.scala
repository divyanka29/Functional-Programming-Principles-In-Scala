...
class Empty extends TweetSet {
    ...
    def mostRetweeted : Tweet = Nil.head
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def descendingByRetweetHelper (curr: TweetSet, acc: TweetList): TweetList =
  {
    if(curr.isEmpty) acc
    else
      {
        val max =  curr.mostRetweeted
        val bigger = descendingByRetweetHelper(curr.remove(max), acc)
        new Cons(max, bigger)
      }
  }
